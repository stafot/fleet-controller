...
func TestShouldHibernate(t *testing.T) {
	...
	t.Run("hibernator can't unlock", func(t *testing.T) {
		shouldHibernate, err := shouldHibernate(installation, userMetrics, mc, false, 7, 100, logger)
	...
	t.Run("installation has no user metrics", func(t *testing.T) {
		mc.newPostCount = 0
		shouldHibernate, err := shouldHibernate(installation, make(map[string]int64), mc, true, 7, 100, logger)
		assert.False(t, shouldHibernate)
		assert.Error(t, err)
		mc.newPostCount = 10
	})
