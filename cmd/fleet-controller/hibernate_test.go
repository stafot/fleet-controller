...
func TestShouldHibernate(t *testing.T) {
	...
	t.Run("hibernator can't unlock", func(t *testing.T) {
		shouldHibernate, err := shouldHibernate(installation, userMetrics, mc, false, 7, 100)
		assert.False(t, shouldHibernate)
		assert.Error(t, err)
	})

	t.Run("installation has new posts", func(t *testing.T) {
		shouldHibernate, err := shouldHibernate(installation, userMetrics, mc, true, 7, 100)
		assert.False(t, shouldHibernate)
		assert.NoError(t, err)
	})

	t.Run("installation has no new posts", func(t *testing.T) {
		mc.installationHasNoNewPosts = true
		shouldHibernate, err := shouldHibernate(installation, userMetrics, mc, true, 7, 100)
		assert.True(t, shouldHibernate)
		assert.NoError(t, err)
		mc.installationHasNoNewPosts = false
	})

	t.Run("installation has no user metrics", func(t *testing.T) {
		mc.installationHasNoNewPosts = true
		shouldHibernate, err := shouldHibernate(installation, make(map[string]int64), mc, true, 7, 100)
		assert.False(t, shouldHibernate)
		assert.Error(t, err)
		mc.installationHasNoNewPosts = false
	})
	...
	t.Run("installation no new posts, but more than maxUsers", func(t *testing.T) {
		mc.installationHasNoNewPosts = true
		shouldHibernate, err := shouldHibernate(installation, userMetrics, mc, true, 7, 4)
		assert.True(t, shouldHibernate)
		assert.Error(t, err)
		mc.installationHasNoNewPosts = false
	})
